...
create or replace function public.campus_set_parent_portal_access(
  account_uuid uuid,
  target_user_id uuid,
  enabled boolean
)
...
begin
  if not exists (
    ...
    raise exception 'Only campus organization owners can manage parent portal access.';
  end if;
...
grant execute on function public.campus_set_parent_portal_access(uuid, uuid, boolean) to authenticated;
